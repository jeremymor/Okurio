...
echo "🚀 AI Builder Stack 2025 - Automated Setup (pnpm)"
echo "=========================================="
echo ""
...
if [ -z "${1:-}" ]; then
    ...
    echo "Usage: bash Setup_AI_Builder_Stack_pnpm.sh my-project-name"
    exit 1
fi
...
if ! pnpm create next-app@latest "$PROJECT_NAME" \
  --typescript \
  --eslint \
  --tailwind \
  --app \
...
if ! pnpm dlx shadcn@latest init -d -y; then
